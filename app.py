...
with tab1:
    ...
    date = st.date_input("Datum", value=pd.Timestamp.now().tz_localize(tz='Europe/Zurich').date())
    time = st.time_input("Uhrzeit", value=pd.Timestamp.now().tz_localize(tz='Europe/Zurich').time())
